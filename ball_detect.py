...
def detect(opt):
    ...
    # Flask socket initalization
    out_address = "127.0.0.1"
    out_port = 6002
    flask_s = socket.socket(socket.AF_INET, # Internet
                    socket.SOCK_DGRAM,socket.IPPROTO_UDP) # UDP
    flask_s.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, MAX_BUFFER) 
    algo_report = {'report_id': 'hoop_algo', 'attempt': 0, 'ball_location': 0}
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    report_flag = True
                    if save_img or opt.save_crop or view_img or report_flag:  # Add bbox to image
                        c = int(cls)  # integer class
                        label = None if opt.hide_labels else (names[c] if opt.hide_conf else f'{names[c]} {conf:.2f}')
                        if names[c] == 'sports ball':
                            xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                            line = (cls, *xywh, conf) # label format
                            print(f'=== >>> Ball is detected : yolo(xywh)={xywh} real(x1,y1,x2,y2)= {torch.tensor(xyxy).view(1,4)} line={line}')
                            attempt +=1
                            algo_report['attempt'] = attempt
                            algo_report['ball_location'] = torch.tensor(xyxy).view(1,4).tolist()
                            json_dumps = json.dumps(algo_report)
            ...
            #img_resize = cv2.resize(im0, (0,0), fx=0.1, fy=0.1)
            img_resize = cv2.resize(im0, (200,200))
            frame = cv2.imencode('.jpg', img_resize)[1].tobytes()
            flask_s.sendto( frame,(out_address, out_port))

            # Stream results
            if view_img:
                im0_r = cv2.resize(im0, (400,400))
                cv2.imshow(str(p), im0_r)
